...
class gnc_api:
    ...
    def set_position_target(self, x, y, z, psi, speed_mps):
        theta = radians((self.correction_heading_g + self.local_offset_g - 90))

        self.position_target.yaw = psi
        ...
        self.position_target.velocity = Vector3()
        self.position_target.velocity.x = speed_mps * unit_pos_vector[0]
        self.position_target.velocity.y = speed_mps * unit_pos_vector[1]
        self.position_target.velocity.z = 0
        # self.position_target.velocity = Vector3(speed_mps, speed_mps, speed_mps)
    ...
    def arm(self):
        ...
        """
        self.set_position_target(0, 0, 0, 0, 0)
    ...
    def check_waypoint_reached(self, pos_tol=10, head_tol=3):
        ...
        dx = abs(
            self.position_target.position.x - self.current_pose_g.pose.pose.position.x
        )
        dy = abs(
            self.position_target.position.y - self.current_pose_g.pose.pose.position.y
        )
        dz = abs(
            self.position_target.position.z - self.current_pose_g.pose.pose.position.z
        )
