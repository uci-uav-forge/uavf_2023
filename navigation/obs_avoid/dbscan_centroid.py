...
def main() -> None:
    ...
    st = time.time()
    centroids, cluster_tup = find_centroids(num_of_clusters, points, labels)
    print(cluster_tup )
...
def find_centroids(num_cluster: int, points: np.ndarray, labels: np.ndarray) -> tuple:
    ...
    centroids = np.zeros((num_clusters, 3), dtype=float)       
    cluster_tup = ()
