...
class Pipeline:
    ...
    def _get_image(self):
        ...
        """
        if self.img_file == "gopro": return self.cam.get_image()
    ...
    def _get_colors_rgb(self, img_crops: "list[np.ndarray]", masks: np.ndarray):
        """
        Returns (shape_colors, letter_colors) where each of those is a list of (ndarrays of shape (3,))
        """
        shape_colors = []
        letter_colors = []
    ...
    def _plot_results(self, loop_index, valid_results, letter_image_buffer, letter_crops, masks):
    ...
    def loop(self, loop_index: int):
        # If profiler needed: https://stackoverflow.com/a/62382967/14587004
        try:
        ...
        if PLOT_RESULT: self._plot_results(loop_index, valid_results, letter_image_buffer, letter_crops, masks)
